...
public class PlayerInteraction : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask rockLayer; // Слой для камней
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (currentRock != null) TryMineRock();
        }
    ...
    private void PickUp()
    {
        if (inventory.AddItem(currentItem.itemData, currentItem.count))
        {
            ...
            currentItem = null; 
            AudioManager.Instance?.PlayPickup(); // Звук подбора предмета
    ...
    private void DropFromHotbar()
    {
        ...
        if (slot.item != null)
        {
            ...
            AudioManager.Instance?.PlayDrop(); // Звук выброса предмета
            inventoryUI.RefreshUI();
        }
    }
    ...
    // Проверяет, смотрим ли на камень
    private void CheckForRocks()
    {
        Ray ray = Camera.main.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f));

        if (Physics.Raycast(ray, out RaycastHit hit, interactDistance, rockLayer))
        {
            MineableRock rock = hit.collider.GetComponentInParent<MineableRock>();
            if (rock != null) { currentRock = rock; return; }
        }

        currentRock = null;
    }

    // Попытка добыть камень
    private void TryMineRock()
    {
        int activeIndex      = inventoryUI.GetActiveHotbarIndex();
        InventorySlot activeSlot = inventory.slots[activeIndex];

        // Проверяем что в руках инструмент
        if (activeSlot.item == null || activeSlot.item.type != ItemType.Tool) return;

        // Передаём имя инструмента и камень сам проверит подходит ли он
        currentRock.TryMine(this, activeSlot.item.itemName);
    }
    }