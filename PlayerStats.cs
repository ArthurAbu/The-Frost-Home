...
public class PlayerStats : MonoBehaviour
{
    ...
    private void UpdateHunger()
    {
        ...
        // Звук голода при уровне ниже 25%
        if (currentHunger / maxHunger <= 0.25f)
        AudioManager.Instance?.TryPlayHungerWarning();
    }
    ...
    private void UpdateWarmth()
    {
        ...
        // Звук холода при уровне ниже 25%
        if (currentWarmth / maxWarmth <= 0.25f)
        AudioManager.Instance?.TryPlayColdWarning();
    }
    ...
    public void TakeDamage(float amount)
    {
        ...
        // Проверяем порог каждые 10% здоровья
        AudioManager.Instance?.CheckHealthThreshold(currentHealth / maxHealth);
    ...
    public void AddFood(float amount)
    {
        ...
        // Сбрасываем предупреждение и звук не сыграет сразу после еды
        AudioManager.Instance?.ResetHungerWarning();
    }
    ...
    public void SetNearFire(bool value)
    {
        isNearFire = value;
         ...
         // Сбрасываем предупреждение холода когда игрок у костра
        if (value) AudioManager.Instance?.ResetColdWarning();
    }
